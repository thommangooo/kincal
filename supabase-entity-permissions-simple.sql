-- Kin Canada Entity Permissions Migration Script (Simple Version)
-- This script adds entity permissions system to existing database
...
-- First, check if the table is empty before inserting
DO $$
BEGIN
    -- Only insert if table is empty
    IF NOT EXISTS (SELECT 1 FROM user_entity_permissions LIMIT 1) THEN
        -- Insert sample club permissions
        INSERT INTO user_entity_permissions (user_email, entity_type, entity_id) 
        SELECT 'demo@example.com', 'club', id FROM clubs LIMIT 3;
        
        -- Insert sample zone permissions
        INSERT INTO user_entity_permissions (user_email, entity_type, entity_id) 
        SELECT 'demo@example.com', 'zone', id FROM zones LIMIT 2;
        
        -- Insert sample district permissions
        INSERT INTO user_entity_permissions (user_email, entity_type, entity_id) 
